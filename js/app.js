...
const TITLE_DEFS=[
  ...
  {k:'third',  cat:'rank',  e:'🥉', n:'삼대장',        c:'#d68f5c', d:'전체 순위 3위 (3판 이상) — 시상대 막차 탑승'},
  // ── 기세 ──
  ...
  {k:'form',   cat:'streak',e:'😤', n:'폼 미쳤다',     c:'#ff7fa0', d:'최근 10판 중 7승 이상 — 지금 폼이 커리어하이'},
  {k:'rocket', cat:'streak',e:'🚀', n:'수직 상승',     c:'#5cdb95', d:'최근 5판 점수 합계 +80 이상 — 브레이크 없음'},
  {k:'fall',   cat:'streak',e:'🪂', n:'자유낙하',      c:'#8fa8c9', d:'최근 5판 점수 합계 -80 이하 — 낙하산은 챙겼니?'},
  // ── 실력 ──
  ...
  {k:'weekend',cat:'life',  e:'🍻', n:'주말 전사',     c:'#e0a35c', d:'토·일 경기 5판 이상 — 주말은 협곡에서'},
  {k:'evening',cat:'life',  e:'🌆', n:'퇴근 후 협곡',  c:'#c98fb8', d:'평일 저녁 18~24시 경기 5판 이상 — 오늘도 정시 퇴근 성공'},
  {k:'legend', cat:'life',  e:'🗿', n:'전설의 고인물', c:'#a8b5c9', d:'내전 100판 이상 참여 — 이 내전의 살아있는 역사'},
  // ── 장인 (라인별) ──
  {k:'topgod', cat:'lane',  e:'🗼', n:'고독한 섬주인', c:'#d6b25f', d:'최근 기록 중 탑으로 10판 이상 — 탑은 원래 1대1 섬'},
  {k:'jggod',  cat:'lane',  e:'🐺', n:'숲의 지배자',   c:'#7fc98f', d:'최근 기록 중 정글로 10판 이상 — 이 정글의 주인은 나'},
  {k:'midgod', cat:'lane',  e:'⚡', n:'협곡의 심장',   c:'#f0d05c', d:'최근 기록 중 미드로 10판 이상 — 게임의 중심'},
  {k:'adcgod', cat:'lane',  e:'🏹', n:'평타의 장인',   c:'#e08f8f', d:'최근 기록 중 원딜로 10판 이상 — 뒤만 봐주면 다 죽임'},
  {k:'supgod', cat:'lane',  e:'💖', n:'헌신의 아이콘', c:'#c9a3e0', d:'최근 기록 중 서폿으로 10판 이상 — 팀을 위해 이 한 몸'},
];
const TITLE_CATS={rank:'순위',streak:'기세',skill:'실력',honor:'명예',life:'생활',lane:'장인'};
...
function titlesOf(p,rank){
  ...
  if(h.length>=10&&h.slice(0,10).filter(x=>x.win).length>=7)t.push(TD.form);
  if(h.length>=5){
    const sum5=h.slice(0,5).reduce((s,x)=>s+(x.d||0),0);
    if(sum5>=80)t.push(TD.rocket);
    if(sum5<=-80)t.push(TD.fall);
  }
  // 실력
  ...
  if(h.filter(x=>{const dy=new Date(x.t).getDay();return dy===0||dy===6;}).length>=5)t.push(TD.weekend);
  if(h.filter(x=>{const dt=new Date(x.t),dy=dt.getDay(),hr=dt.getHours();return dy>=1&&dy<=5&&hr>=18;}).length>=5)t.push(TD.evening);
  if(g>=100)t.push(TD.legend);
  // 장인 (라인별 — 최근 기록 30판 기준)
  {const cnt={}; h.forEach(x=>{cnt[x.pos]=(cnt[x.pos]||0)+1;});
   if((cnt.TOP||0)>=10)t.push(TD.topgod);
   if((cnt.JG ||0)>=10)t.push(TD.jggod);
   if((cnt.MID||0)>=10)t.push(TD.midgod);
   if((cnt.ADC||0)>=10)t.push(TD.adcgod);
   if((cnt.SUP||0)>=10)t.push(TD.supgod);}
  return t;
}
...
//============ 헥스테크 마법가루 (배경 파티클 + 승리 폭죽) ============
// 은은히 떠오르는 골드 가루. 40fps 상한·탭 숨김 시 정지·모션 최소화 설정 존중 — 배터리 걱정 없음.
(function(){
  if(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;
  const cv=document.createElement('canvas');
  cv.id='fxCanvas'; cv.setAttribute('aria-hidden','true');
  document.body.prepend(cv);
  const ctx=cv.getContext('2d');
  let W=0,H=0;
  const parts=[],bursts=[];
  const HUES=['244,216,150','200,170,110','244,216,150','150,205,195']; // 골드 3 : 헥스테크 틸 1
  function mk(fresh){return {
    x:Math.random()*W, y:fresh?Math.random()*H:H+12,
    r:.6+Math.random()*1.7,
    s:.1+Math.random()*.3,
    a:Math.random()*Math.PI*2, sw:.15+Math.random()*.5,
    c:HUES[(Math.random()*HUES.length)|0],
    tw:Math.random()*Math.PI*2,
  };}
  function size(){
    const dpr=Math.min(2,window.devicePixelRatio||1);   // 레티나 폰에서도 또렷하게 (최대 2배)
    W=innerWidth; H=innerHeight;
    cv.width=W*dpr; cv.height=H*dpr;
    ctx.setTransform(dpr,0,0,dpr,0,0);
    const n=Math.min(64,Math.round(W*H/26000));   // 화면 크기에 비례 — 폰에선 10개 안팎, 최대 64개
  ...
  function tick(t){
    ...
    if(t-last<25)return; last=t;                  // ~40fps
    ctx.clearRect(0,0,W,H);
    for(const p of parts){
      p.y-=p.s; p.a+=.008; p.tw+=.03;
      if(p.y<-12)Object.assign(p,mk(false));
      const x=p.x+Math.sin(p.a)*14*p.sw;
      const al=.12+.2*(.5+.5*Math.sin(p.tw));
      ctx.beginPath(); ctx.arc(x,p.y,p.r,0,7);
      ctx.fillStyle='rgba('+p.c+','+al.toFixed(3)+')';
      ctx.shadowColor='rgba('+p.c+',.8)'; ctx.shadowBlur=6;
      ctx.fill(); ctx.shadowBlur=0;
    }
    for(let i=bursts.length-1;i>=0;i--){
      const b=bursts[i]; b.life--;
      if(b.life<=0){bursts.splice(i,1);continue;}
      b.x+=b.vx; b.y+=b.vy; b.vy+=.05; b.vx*=.985;
      const al=Math.max(0,b.life/b.max);
      ctx.beginPath(); ctx.arc(b.x,b.y,b.r*al+.4,0,7);
      ctx.fillStyle='rgba('+b.c+','+al.toFixed(3)+')';
      ctx.shadowColor='rgba('+b.c+',.9)'; ctx.shadowBlur=8;
      ctx.fill(); ctx.shadowBlur=0;
    }
  }
  requestAnimationFrame(tick);
  // 승리 반영 순간 골드 폭죽 (승리 팀 색이 섞임)
  window.__fx={
    burst(team){
      const cx=W/2, cy=Math.min(H*.4,380);
      const tc=team==='red'?'233,120,110':'110,230,215';
      for(let i=0;i<70;i++){
        const an=Math.random()*Math.PI*2, sp=1.2+Math.random()*3.6;
        bursts.push({x:cx,y:cy,vx:Math.cos(an)*sp,vy:Math.sin(an)*sp-1.2,
          r:1+Math.random()*2.2, c:i%3?'244,216,150':tc,
          life:60+Math.random()*45, max:100});
      }
    }
  };
})();
